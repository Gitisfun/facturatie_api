...
class StatisticsQuery {
    ...
    static QUERY_PER_DAY(list, table_name, mode, type){
        ...
        for(let i = 0; i < list.length; i++){
            query += `(SELECT ${SELECT} FROM ${table_name} WHERE bedrijfs_id = ? AND isActive = 1 AND DATE(datum) = ?${WHERE}) AS "${i + 1}"`
    ...
    static QUERY_PER_DAY_RANGE(list, table_name, mode, type){
        const SELECT = StatisticsStates.getSelectQuery(mode)
        const WHERE = StatisticsStates.getWhereQuery(type)

        let query = "SELECT "
        for(let i = 0; i < list.length; i++){
            query += `(SELECT ${SELECT} FROM ${table_name} WHERE bedrijfs_id = ? AND isActive = 1 AND datum BETWEEN ? AND ?${WHERE}) AS "${i + 1}"`
