...
function QUERY_GET_ALL(){
    ...
    const JOIN_GEBRUIKERS = `INNER JOIN ${TABLE_JOIN_GEBRUIKER} ON ${TABLE_NAME}.${COLUMN_UPDATED_BY} = ${TABLE_JOIN_GEBRUIKER}.${COLUMN_GEBR_ID} `;
    const WHERE = `WHERE ${TABLE_NAME}.${COLUMN_IS_ACTIVE} = 1 AND ${TABLE_NAME}.${COLUMN_BEDRIJF_ID} = ?`
    return QUERY + QUERY_JOIN_GEBRUIKER + JOIN_GEBRUIKERS + WHERE
}
...
function QUERY_GET() {
    ...
    const QUERY_JOIN_GEBRUIKER = `${TABLE_JOIN_GEBRUIKER}.${COLUMN_GEBR_ID} "${ALIAS_GEBR_ID}", ${TABLE_JOIN_GEBRUIKER}.${COLUMN_GEBR_VOORNAAM} "${ALIAS_GEBR_VOORNAAM}", ${TABLE_JOIN_GEBRUIKER}.${COLUMN_GEBR_ACHTERNAAM} "${ALIAS_GEBR_ACHTERNAAM}" FROM ${TABLE_NAME} `
    const JOIN_BTW = `INNER JOIN ${TABLE_JOIN_BTW} ON ${TABLE_NAME}.${COLUMN_BTW_ID} = ${TABLE_JOIN_BTW}.${COLUMN_BTW_BTW_ID} `;
    const JOIN_BETALINGSTERMIJN = `INNER JOIN ${TABLE_JOIN_BET} ON ${TABLE_NAME}.${COLUMN_BETALINGSTERMIJN_ID} = ${TABLE_JOIN_BET}.${COLUMN_BET_ID} `;
    const JOIN_GEBRUIKERS = `INNER JOIN ${TABLE_JOIN_GEBRUIKER} ON ${TABLE_NAME}.${COLUMN_UPDATED_BY} = ${TABLE_JOIN_GEBRUIKER}.${COLUMN_GEBR_ID} `;
...
function QUERY_CREATE() {
    const INSERT = `INSERT INTO ${TABLE_NAME} (${COLUMN_LEVERANCIERS_NR}, ${COLUMN_NAAM}, ${COLUMN_EXTRA_NAAM}, ${COLUMN_TELEFOON}, ${COLUMN_EMAIL}, ${COLUMN_BTW_ID}, ${COLUMN_BTW_NR}, ${COLUMN_HANDELREGISTER_NR}, ${COLUMN_ONDERNEMINGSTYPE}, ${COLUMN_TAAL}, ${COLUMN_TYPE}, ${COLUMN_BETALINGSTERMIJN_ID}, ${COLUMN_ADRESSEN}, ${COLUMN_UPDATED_BY}, ${COLUMN_BEDRIJF_ID}) `
    const VALUES = `VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    return INSERT + VALUES
}
  
function QUERY_UPDATE() {
    const UPDATE = `UPDATE ${TABLE_NAME} `
    const SET = `SET ${COLUMN_LEVERANCIERS_NR} = ?, ${COLUMN_NAAM} = ?, ${COLUMN_EXTRA_NAAM} = ?, ${COLUMN_TELEFOON} = ?, ${COLUMN_EMAIL} = ?, ${COLUMN_BTW_ID} = ?, ${COLUMN_BTW_NR} = ?, ${COLUMN_HANDELREGISTER_NR} = ?, ${COLUMN_ONDERNEMINGSTYPE} = ?, ${COLUMN_TAAL} = ?, ${COLUMN_TYPE} = ?, ${COLUMN_BETALINGSTERMIJN_ID} = ?, ${COLUMN_ADRESSEN} = ?, ${COLUMN_UPDATED_BY} = ?, ${COLUMN_UPDATED_TIME} = NOW(), ${COLUMN_BEDRIJF_ID} = ? `
    const WHERE = `WHERE ${COLUMN_ID} = ?`
    return UPDATE + SET + WHERE
}
  
function QUERY_DELETE() {
    const UPDATE = `UPDATE ${TABLE_NAME} `
    const SET = `SET ${COLUMN_IS_ACTIVE} = 0 `
    const WHERE = `WHERE ${COLUMN_ID} = ?`
    return UPDATE + SET + WHERE
}
...
function QUERY_ESP(){
    const UPDATE = `UPDATE ${TABLE_NAME} `
    const SET = `SET ${COLUMN_IS_BLACKLISTED} = ? `
    const WHERE = `WHERE ${COLUMN_ID} = ?`
    return UPDATE + SET + WHERE 
}

export { QUERY_GET, QUERY_GET_ALL, QUERY_CREATE, QUERY_UPDATE , QUERY_DELETE, QUERY_ESP };