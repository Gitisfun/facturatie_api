...
const COLUMN_CREDITNOTAS = "creditnotas"
...
function QUERY_GET() {
    const QUERY = `SELECT ${COLUMN_ID}, ${COLUMN_AANKOPEN}, ${COLUMN_VERKOPEN}, ${COLUMN_CREDITNOTAS} FROM ${TABLE_NAME} `
...
function QUERY_UPDATE() {
    const COLUMNS = [COLUMN_AANKOPEN, COLUMN_VERKOPEN, COLUMN_CREDITNOTAS]
...
function QUERY_INCREMENT(table){
    switch(table){
        ...
        case TableStates.CREDITNOTAS:
            return `UPDATE ${TABLE_NAME} SET ${COLUMN_CREDITNOTAS} = ${COLUMN_CREDITNOTAS} + 1`
        default:
            return ""
    }
}
